...
void loop() {
  ...
  if (!ball.found || (ball.found && ball.dist > 75)) {
    speed_x = constrainAngle(home.dir - PI) * 400;
  } else {
    if (abs(home.dir) > 2.7) {
      speed_x = -ball.dir * 400;
    } else {
      speed_x = 0;
    }
//    if (abs(home.dir) < 2.7 && abs(ball.dir) > PI / 3)
//      speed_x = 0;

  }

  if (abs(speed_x) < 200) {
    speed_y = (home.dist - 55) * 50;
  }
  ...
  if (speed_y) {
    dir = atan(speed_x / speed_y) + PI / 2 + PI / 2 * sign(speed_y);
  } else {
    dir = PI / 2 * -sign(speed_x);
  }

  speed = sqrt(speed_x * speed_x + speed_y * speed_y);

  if (ball.found && ball.dist < 60 && home.dist < 80 && abs(ball.dir) < PI / 4)  {
    dir = ball.dir;
    speed = 300;
//    if (abs(ball.dir) < PI / 4) {
//      dir = ball.dir * 2;
//      speed = 255;
//    }
  }

  if (ball.x > 190 && ball.x < 197 && ball.y > 130 && ball.y < 135) {
    cBallTimer = 0;
    heading = target.dir;
    speed = 300;
  } else {
